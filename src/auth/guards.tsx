 ...
 * Super admins always pass; instructors are checked against the configured list.
...
export function RequireModule({ moduleKey, children }: { moduleKey: string; children: ReactElement }) {
  ...
  if (!isAuthed) return <Navigate to="/login" replace />;
  if (!profile) return <Spinner label="Loading your profile…" />;
  if (!['instructor', 'super_admin'].includes(profile.role)) return <Navigate to="/app" replace />;
