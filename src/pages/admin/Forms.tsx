...
import { Copy, FormInput, ListChecks, Plus, Settings2 } from 'lucide-react';
...
export function Forms() {
    ...
    const [forms, courses] = await Promise.all([
      unwrap(
        supabase
          ...
          .select('id, title, slug, is_open, course:courses(title), enrollment_form_fields(id), enrollment_requests(id)')
  ...
  const copyUrl = (slug: string) => {
    navigator.clipboard.writeText(`${APP_URL}/enroll/${slug}`);
          ...
          {q.data.forms.map((f) => (
            <GlassCard key={f.id} className="p-5">
              ...
              <div className="mt-2 flex items-center gap-1.5 text-xs text-neutral-500">
                <span className="font-mono">/enroll/{f.slug}</span>
                <button onClick={() => copyUrl(f.slug)} className="text-neutral-400 hover:text-blue-600">
                  <Copy size={13} />
                </button>
              </div>
...
function CreateForm({ courses, createdBy, onDone }: { courses: any[]; createdBy: string; onDone: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('enrollment_forms').insert({ course_id: courseId, title, description, slug, created_by: createdBy });
    setBusy(false);
    if (error) return toast(error.message, 'error');
    toast('Form created');
    onDone();
